...
### Here we simulate data from a linear relationship between the exposure and the outcome (SBP), 
### then dichotize SBP to obtain a binary outcome (hypertension),
...
### This first part of the code comes in 3 variants: using a linear relation, using a quadratic relation, and using a linear relation and then 
### dichotomize the outcome and apply logistic regression in stead of linear regression (this code)


### Berkson error is calculation from error prone x using a prediction model

### Fixed categories made small as otherwise lot of data
### fall outside outer categories in case of Berkson error
   
  require(foreach)
  library(doParallel)
  cl<-makeCluster(3, outfile = "debuglog.txt")
  registerDoParallel(cl)  
  
  rm(sim.dt) # remove old versions 
  ...
  # foreach(xx = 1:3) %dopar% {
    ...
    nrec <- 2000  # number of records in a single dataset : larger than for continuous in order
    ## to have enough positive cases in each category we use N=2000 in stead of n=500
    distmean <- 100 # mean value of the exposure
    sigma <-   0.24 # standard deviation of the exposure in the population on the logscale
    ...
    sdexp <- distmean*sqrt(exp(sigma^2)-1) ### we use the 
    sdexp <- sd(exptrue)
    
    
    ## simulate another predictor for the outcome 
    ## confounder1 and 2 are confounders, and confounder0 is not a confounder
    ## make them on average 0 and give them a standard deviation of 1
    ...
    ## confounder1: confounder that is positively associated with exposure
    ## confounder1 is confounder with same distribution as confounder0 and the same variance
    ## and the same predictive value
    ## the 0.02 below causes a correlation with the exposure of slightly over 0.5   
    ## If it is changed to 0.033 the correlation increase from 0.5 to 0.9 approximately
    ## The correlation is slightly different for depending on the distribution of exptrue
    
    confounder1 <-
      sqrt(1-(0.02*sdexp)^2)*rnorm(n, 0, 1) + exptrue * 0.02 - mean(exptrue)* 0.02 
   
    ##  confounder2: confounder of equal influence but but negative correlation with exposure
    ##  if also has a mean of 0 and variance of 1 
    confounder2 <-
    ...
    cbind(sd(confounder0),sd(confounder1),sd(confounder2))
    ...
    ### SBP2: strong effect of exposure
     ...
     ###################################################################################
     ###
     ### hypertenstion variables: aim: around 30%
     ### 
     ###################################################################################
     
     
    ### intercept is 134 to increase percentage of hypertension
    ...
    ## confounder 1: less strong relation
    ...
    summary(SBP2C) ## confounder 1: strong relation
    ...
    vartoadd <-  91-(0.1*sdexp)^2 - 2*cov(3*confounder2,0.1*exptrue)
    SBP2C2 <-
    ...
    ## JUST FOR CHECKING
    SBP2marginal <-
      134 - 0.1*mean(exptrue) +0.1* rnorm(n, 100,  sd(exptrue))+  3 * (confounder2) + rnorm(n, 0, 1) ## strong effect of exposure and confounding
    cor(SBP2marginal,confounder2 )
 
    ### null model
  
    vartoadd <- 91
    SBP0C2 <-
      134 + 5 * (confounder2) + rnorm(n, 0, sqrt(vartoadd)) ## no effect of exposure and confounding
    mean(SBP0C2>139)
    ...
    x3 <-
      pmax(x2, 0) ## truncated large error (x always positive) ## not yet used
    ## print some results
    cor(x1, exptrue)
    cor(x2, exptrue)
    ...
    ### make hypertension variables but keep the same name (SBP)
    ...
    nError <- 7  ## different error types (including none)
      ...
      for (confounder in c(0, 1, 2)) {
        for (association in c(0, 1, 2)) {
          for (cattype in 1:2) {
            ...
            err <- 1
            for (err in 0:6) {
              # in previous version err=0 was choosen
              # for no error
              ...
              modres2 <- gamlss(resref ~ pbm(res, mono="up") + c, data=data4gamlss,control = gamlss.control(trace=FALSE))
                ## this can give outlier points for some datasets 
  ...
  ### run the models (naive and on calibrated data)
              modobs.error <-
                summary(glm(paste(SBP, "~ catx +", C),family = "binomial", data = loop.dt))[[12]] ## coefficients
              modobs.cal <-
                summary(glm(paste(SBP, "~ catcal +", C),family = "binomial", data = loop.dt))[[12]] ## coefficients
              if ( sum(!is.na(meanxcalres))>1)  modobs.calres <-
                summary(glm(paste(SBP, "~ catcalres + ", C), family = "binomial", data = loop.dt))[[12]] 
              if ( sum(!is.na(meanxcalspline))>1)  modobs.calspline <-
                summary(glm(paste(SBP, "~ catcalspline + ", C), family = "binomial", data = loop.dt))[[12]] ## coefficients
              if ( sum(!is.na(meanxcalresspline))>1)  modobs.calresspline <-
                summary(glm(paste(SBP, "~ catcalresspline + ", C), family = "binomial", data = loop.dt))[[12]] ## coefficients
   ...
   ### this has no place in the file, is also redundant because same as error = 0 
              ...
              ## is not completely right name, because this is for the mean of the category, which is not the mean of all points within
              ## the category
              
              
              
              ##### needs change for linear /quadratic 
      ...
      ### method 15
              ...
              mod.cont.error <-
                summary(glm(paste(SBP, "~", X, " + ", C), family = "binomial", data = loop.dt))[[12]]
              mod.cont.cal <-
                summary(glm(paste(SBP, "~ cali + ", C), family = "binomial", data = loop.dt))[[12]]
    ...
    save(file = paste0("resultssim_log9", xtype,".RData"), results)
